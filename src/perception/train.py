...
category_dict = {
  ...
  7: 'no heavy truck',
  8: 'no parking',
  9: 'no stopping and parking',
  10:'residential',
  11:'narrows from left',
  12:'narrows from right',
  13:'roundabout',
  14:'stop'
}


def train(device="cpu", batch_size=8):
    """Train the network.
    ...
    weight_classif = wandb.config.weight_classif = 1
    ...
    while current_iteration <= max_iterations:
        for img_batch, target_batch in dataloader:
            ...
            predicted_classes = torch.argmax(out[pos_indices[0], 5:20, pos_indices[1], pos_indices[2]], dim=1)
            actual_classes = torch.argmax(target_batch[pos_indices[0], 5:20, pos_indices[1], pos_indices[2]], dim=1)
            accuracy = float(torch.eq(predicted_classes, actual_classes).sum().item()) / float(len(pos_indices[0]))

            #Calculate classification accuracy
            #correct = 0
            #for i in range(len(label_batch)):
              #correct += (torch.argmax(logits[i], dim=0) == label_batch[i]).sum().item()

            wandb.log(
                {
                    "total loss": loss.item(),
                    "total loss with classif": tot_loss.item(),
                    "loss pos": pos_mse.item(),
                    "loss neg": neg_mse.item(),
                    "loss reg": reg_mse.item(),
                    "loss classif": classif_mse.item(),
                    "classif accuracy": accuracy,
                },
                step=current_iteration,
            )

            print(
                "\rIteration: {}, loss: {} total_loss: {}".format(current_iteration, loss.item(), tot_loss.item()),
                end="",
            )
            ...
            if current_iteration % 2000 == 0:
              model_path = "{}iter{}.pt".format(run_name, current_iteration)
