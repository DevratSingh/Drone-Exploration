...
class LocalFrontier(Planner):
    ...
    def __init__(self, start, goal, iterations, res, inflateradius, obstacles, ext, args, map_res = 0.01, radius = 30, maxdist = 0.2, path_res = 0.2, sample_rate = 5):
        

        self.point_pub = rospy.Publisher('localfrontpt', PoseStamped, queue_size=10)
        self.grid_pub = rospy.Publisher('gridmap', OccupancyGrid, queue_size=10, latch=True)
        self.gridupdate_pub = rospy.Publisher('gridmap_updates', OccupancyGridUpdate, queue_size=10)
        self.frontpt_pub = rospy.Publisher('locfrontnodes', MarkerArray, queue_size=10)
    ...
    def frontier(self):
        ...
        self.grid = self.markexplored(self.initNode.x,self.initNode.y)

        prevlocation = self.Node(0,0,0)
        ...
        while not rospy.is_shutdown():
            ...
            if self.gridCheck(tempNode):
                ...
                msg = displaypathNodes(displaynodes,[1,1,0])
                ...
                #Re-initialize the vertices and edges to current location
                locNode = self.getLocation()
                if locNode is not None:
                    self.vertices.append(locNode)
                    self.grid = self.markexplored(locNode.x,locNode.y)
                    
                    prevlocation = locNode
                else:
                    self.vertices.append(prevlocation)
    ...
    def markexplored(self, x, y):
        ...
        if self.grid_map.is_in_bounds(x,y):
            ...
            data = np.zeros((2*self.radius)**2)

            for k in range(i - self.radius, i + self.radius):
                for l in range(j - self.radius, j + self.radius):
                    #temp.append(self.grid_map.indextocoord(k, l))

                    if ((i-k)**2 + (j-l)**2)**0.5 <= self.radius:
                        if not(self.map_matrix[k, l] >= 50):                                 
                            self.map_matrix[k][l] = 30
        
        #d =  self.map_matrix.flatten('C')
        #self.frontpt_pub.publish(displaypathNodes(temp,[1,0,0]))
    ...
    def getLocation(self):
        ...
        while not rospy.is_shutdown():
            try:
                ...
                #rate.sleep()
                rospy.loginfo("[Local]: No TF found, trying again")
                #return None
...
def main(argv=sys.argv):
    ...
    rospy.init_node('localfrontier', anonymous=True)
    ...
    while not rospy.is_shutdown():
        try:
            ...
            rospy.loginfo("[local]: No TF found")
            rate.sleep()
            continue
            

    localfront = LocalFrontier(start, prevgoal, 100, res, inflateradius, obstacles, ext, args)
    localfront.frontier()
