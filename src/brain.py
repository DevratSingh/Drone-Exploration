...
def posestamped2position(pose):
    ...
    position.z = 0.4#pose.pose.position.z#0.4
...
def tfstamped2position(tf):
    ...
    position.z = 0.4#tf.transform.translation.z
...
def path_callback(msg, d = wp_reach_dist):
    ...
    if msg.poses:
        try:
            ...
            lastpoint = None
        i = 1
        for wp in reversed(msg.poses): # for each waypoint in the path
            ...
            while not distance2wp(waypoint) < d:
                rate.sleep()
            rospy.loginfo("[Brain]: Reached waypoint %i", i)
            i = i + 1
    else:
...
stop_pub = rospy.Publisher('/cf1/cmd_stop', Empty, queue_size=2)
pub_cmd = rospy.Publisher('/cf1/cmd_position', Position, queue_size=2)
pub_hover = rospy.Publisher('/cf1/cmd_hover', Hover, queue_size=2)
pub_vel = rospy.Publisher('/cf1/cmd_vel', Twist, queue_size=2)
